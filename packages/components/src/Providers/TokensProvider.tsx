...
import { formatCss } from "../utils/formatCss.ts";
import { useInjectGlobalStyles } from "./useInjectGlobalStyles.tsx";
...
export function TokensProvider({scope}: TokensProviderProps) {
    const tokens = useMemo(() => {
        return {
            ...
            lightSemantic: scope === "hop-0-0-0" ? lightSemanticTokens(scope): alternativeLightSemanticTokens(scope),
            darkSemantic: scope === "hop-0-0-0" ? darkSemanticTokens(scope): alternativeDarkSemanticTokens(scope)
        }
    }, [scope])
...
// Take tokens and generate this format

const coreTokens  = (scope: string) => formatCss(`
.hop.${scope} {
     --o-ui-sp-1: .25rem;
     --o-ui-sp-2: .5rem;
     --o-ui-sp-3: .75rem;
...
const lightSemanticTokens = (scope:string) => formatCss(`
.hop.${scope}-light {
...
const darkSemanticTokens = (scope:string) => formatCss(`
.hop.${scope}-dark {
...
const alternativeLightSemanticTokens = (scope:string) => formatCss(`
.hop.${scope}-light {
...
const alternativeDarkSemanticTokens = (scope:string) => formatCss(`
.hop.${scope}-dark {
